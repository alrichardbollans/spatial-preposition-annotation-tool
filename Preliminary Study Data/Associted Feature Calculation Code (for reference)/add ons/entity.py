#This code is originally from https://github.com/gplatono/SRP and has been modified for our needs
...
class Entity:
	...
	### Creates Convex Hull Object
	# def get_convex_hull(self):

	# 	bpy.context.screen.scene = self.scene
	# 	if 'convexhull' not in self.name:

	# 		bpy.ops.object.select_all(action='DESELECT')

	# 		### Create a duplicate
	# 		self.constituents[0].select = True

	# 		bpy.context.scene.objects.active = self.constituents[0]
	...
	# 		# Rename
	# 		bpy.context.active_object.name = self.name + "convexhull"
			
	# 		#Deselect
	# 		self.constituents[0].select = False

	# 		return self.scene.objects[self.name + "convexhull"]
